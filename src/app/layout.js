...
export const metadata = {
  ...
  description: 'Fikiara is a platform for learning and personal transformation through books, videos, and courses inspired by many types books.',
  keywords: [
    'Fikiara',
    'Quran learning',
    'Life transformation',
...
export async function generateSitemap() {
  ...
  const urls = [
    ...
    ...books.map(book => ({ url: `${baseUrl}/books/${book.slug}`, lastModified: new Date(book.updatedAt) })),
    ...videos.map(video => ({ url: `${baseUrl}/videos/${video.slug}`, lastModified: new Date(video.updatedAt) })),
    ...courses.map(course => ({ url: `${baseUrl}/courses/${course.slug}`, lastModified: new Date(course.updatedAt) })),
  ];
  ...
  return (
    <html lang="en">
      <head>
        ...
        <meta name="google-site-verification" content="UUMrfZueowRoy1izs0c_aXKUtn2LYYkNJbf2N9i3uhA" />
